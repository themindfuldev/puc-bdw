...
###############################################################################
{
  return '0.2.2b'; # AW mods for trailing slash, entities
} # function kses_version
...
###############################################################################
{
  ...
  if (!@isset($allowed_html[strtolower($elem)]))
    return ' ';  # AW: replace disallowed strings with spaces
  ...
  foreach ($attrarr as $arreach)
  {
    ...
    # there are some checks
      ...
      foreach ($current as $currkey => $currval)
        if (true !== ($ok = kses_check_attr_val($arreach['value'], $arreach['vless'],
                                               $currkey, $currval)))
        {
          if ($ok == -1)
            return '';  # deny the entire element
        }
...
###############################################################################
{
  ...
  switch (strtolower($checkname))
  {
    ...
    case 'xminval': # AW
    # The xminval check checks that the attribute value is a positive integer,
    # and that it is not smaller than the given value.  Otherwise, the entire
...
# AW: dont require trailing ;, since some browsers are lax about it
###############################################################################
{
  $string = preg_replace('/&#([0-9]+);?/e', 'chr("\\1")', $string);   # AW
  $string = preg_replace('/&#[Xx]([0-9A-Fa-f]+);?/e', 'chr(hexdec("\\1"))', # AW
                         $string);
